...
def download_and_copy_dependencies(helper_args: BuildHelperArgs):
    ...
    if _normalize_bool(os.getenv("TRITON_BUILD_PROTON", "ON")):  # Default ON
        ...
        download_and_copy(
            name="nvidia/cuda-stdlib-" + nvidia_toolchain_version["cuda-stdlib"],
            src_func=lambda system, arch, version: f"cuda_cccl-{system}-{arch}-{version}-archive/include/cccl/cuda",
            dst_path="third_party/nvidia/backend/include/cuda",
            override_path=helper_args.cuda_stdlib_path,
            version=nvidia_toolchain_version["cuda-stdlib"],
            url_func=lambda system, arch, version:
            f"https://developer.download.nvidia.com/compute/cuda/redist/cuda_cccl/{system}-{arch}/cuda_cccl-{system}-{arch}-{version}-archive{archive_extension}",
            helper_args=helper_args,
        )
        download_and_copy(
            name="nvidia/cuda-stdlib-" + nvidia_toolchain_version["cuda-stdlib"],
            src_func=lambda system, arch, version: f"cuda_cccl-{system}-{arch}-{version}-archive/include/cccl/nv",
            dst_path="third_party/nvidia/backend/include/nv",
            override_path=helper_args.cuda_stdlib_path,
            version=nvidia_toolchain_version["cuda-stdlib"],
            url_func=lambda system, arch, version:
            f"https://developer.download.nvidia.com/compute/cuda/redist/cuda_cccl/{system}-{arch}/cuda_cccl-{system}-{arch}-{version}-archive{archive_extension}",
